<template>
  <section
    class="min-w-0 w-full rounded-[2rem] border shadow-xl overflow-hidden"
    :class="
      isNightMode
        ? 'bg-[#15131a] border-[#4a3d62] text-white'
        : 'bg-[#fbf8ff] border-[#ddd2ec] text-gray-800'
    "
  >
    <template v-if="mode === 'list'">
      <div class="border-b p-5" :class="isNightMode ? 'border-white/10' : 'border-[#e7deef]'">
        <div class="flex flex-col gap-3 lg:flex-row lg:items-center">
          ...
          <button
            class="rounded-xl bg-sky-600 px-5 py-3 font-bold text-white"
            @click="startCreate()"
          >
            + 写随笔
          </button>
      ...
      <div class="p-6">
        <div v-if="filteredEssays.length" class="grid gap-4 md:grid-cols-2 2xl:grid-cols-3">
          ...
          >
            ...
            <p class="mt-3 line-clamp-4 text-sm leading-6 opacity-65">
              {{ textPreview(note.content) }}
            </p>
            <div class="mt-4 text-xs font-bold opacity-45">
              {{ note.wordCount || 0 }} 个有效字符
            </div>
          </button>
        </div>
    ...
    <template v-else>
      <div class="border-b px-6 py-4" :class="isNightMode ? 'border-white/10' : 'border-[#e7deef]'">
        <div class="flex flex-wrap items-center justify-between gap-3">
          <button class="text-sm font-bold opacity-70 hover:opacity-100" @click="closeEditor">
            ← 返回随笔列表
          </button>
      ...
      <div v-if="mode === 'view' && currentNote" class="min-w-0 p-6 lg:p-8">
        <div class="mx-auto max-w-4xl">
          ...
          <div class="mt-2 text-xs opacity-50">
            最后更新：{{ displayDate(currentNote.updatedAt || currentNote.date) }}
          </div>
          <div class="mt-8">
            <MarkdownPreview :source="currentNote.content" :is-night-mode="isNightMode" />
          </div>
        </div>
      </div>
      ...
      <div v-else>
        <div
          class="grid grid-cols-2 gap-2 border-b p-3 lg:hidden"
          :class="isNightMode ? 'border-white/10' : 'border-[#e7deef]'"
        >
          <button
            class="rounded-xl px-4 py-2 text-sm font-bold"
            :class="mobilePane === 'edit' ? 'bg-sky-600 text-white' : 'border'"
          ...
          <button
            class="rounded-xl px-4 py-2 text-sm font-bold"
            :class="mobilePane === 'preview' ? 'bg-sky-600 text-white' : 'border'"
          ...
          >
            ...
            <div class="grid gap-3 sm:grid-cols-2">
              ...
              >
                ...
                <option v-for="action in actions" :key="action.id" :value="String(action.id)">
                  {{ action.name }}
                </option>
              </select>
              <select
...
const essays = computed(() =>
  notebook.value
    ...
    .sort(
      (left, right) =>
        new Date(right.updatedAt || right.date) - new Date(left.updatedAt || left.date),
    ),
)
const currentNote = computed(
  () => essays.value.find((note) => note.id === currentNoteId.value) || null,
)
...
function beginEdit() {
  ...
  draft.actionId =
    currentNote.value.actionIds?.[0] == null ? '' : String(currentNote.value.actionIds[0])
...
function saveEssay() {
  ...
  if (currentNoteId.value) {
    ...
    const created = notebookStore.createEssayNote(
      draft.title.trim(),
      draft.content.trim(),
      actionIds,
      { skillId },
...
async function requestLeave() {
  ...
  const choice = await choiceDialog(
    '当前随笔有尚未保存的修改。保存后离开、放弃修改，还是继续编辑？',
    {
      title: '处理未保存修改',
      cancelText: '继续编辑',
      choices: [
...
function ownerLabel(note) {
  const action = actions.value.find((item) =>
    note.actionIds?.some((id) => String(id) === String(item.id)),
  )
