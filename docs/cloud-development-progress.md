...
|---|---|---|---|---|---|---|---|
| V050-G01～G05、S01～S08、AC01～AC08 | 稳定编号的版本边界、流程、状态、接口和验收规格 | 已完成 | `docs/v0.5-cloud-spec.md` | L1：目标用户、在线优先、非目标、外部依赖和恢复路径已闭合 | 项目管理 | 真实账号参数仍须部署时回填 | 部署完成后记录真实资源引用 |
...
| V050-S02～S03、S08、AC02、AC07 | Worker Access JWT、同源校验、载荷限制、结构化错误与安全日志 | 已完成 | `worker/auth.mjs`、`validation.mjs`、`responses.mjs`、`index.mjs` | L2：真实 Workers 运行时测试覆盖 API 安全边界；静态检查通过 | Cloudflare Workers | Access JWT 仍需真实 team domain/audience 验证 | 在预览环境检查未登录、过期登录和日志脱敏 |
...
| V050-A02、AC04、AC07～AC08 | Cloudflare 真实预览资源、Access 允许名单、迁移与部署 | 进行中 | `wrangler.jsonc` preview 环境；远程资源见下方部署记录 | L3：preview D1 创建并执行迁移；Worker/19 个静态资源已部署；远程首页 `200`、health 为 `preview`、未配置 Access 的 snapshot API 以 `500` 拒绝 | Cloudflare 账号 | Wrangler 最小 OAuth 不含 Zero Trust Access 编辑；需要用户在应用内浏览器登录控制台后继续 | 创建 Self-hosted Access 应用，回填 team domain/audience，再部署验证 |
...
| URL | `https://minerva-ranger-cloud-preview.feiyut666.workers.dev` | 首页与 health 可用；尚未由 Access 保护 |
| Version | `2f4e1dc3-b859-4013-bf01-7d27c3c603c3` | 首次 preview 版本 |
| Access | Self-hosted application | 待创建；`TEAM_DOMAIN`、`POLICY_AUD` 和允许名单尚未回填 |
...
- Cloudflare Access team domain、audience 和允许名单尚未绑定；preview D1 与 Worker 已部署，但在 Access 完成前不能声明远程可邀请试用。
