...
| V040-S01、S06、A01、T01～T02、AC07 | 主进程数据库、模式迁移、preload 与 IPC 安全边界 | 已完成 | `electron/persistence/`、`electron/preload.js`、`electron/main.js`、`index.html` | L2：模式、错误、IPC sender 测试；L3：真实 preload bridge 与 SQLite 健康检查；静态检查通过 | Electron/Windows | 当前使用 `file://` + 严格 sender/path 校验；后续可评估自定义协议 | 保持 preload API 用例化，不暴露任意 IPC/SQL |
